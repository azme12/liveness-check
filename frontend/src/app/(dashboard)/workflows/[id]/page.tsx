...
export default function WorkflowDetailPage() {
  ...
  async function createVersion() {
    ...
    try {
      ...
      window.location.assign(`/workflows/${wf.id}/versions/${created.id}`);
    } catch (err) {
  ...
  if (error && !wf) {
    return (
      <div className="space-y-3">
        ...
        <Link href="/workflows" className="text-sm text-[var(--accent)] underline">
          ← Back to workflows
        </Link>
      </div>
    );
            ...
            {(wf.versions || []).map((v) => (
              <tr key={v.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-hover)]">
                <td className="px-4 py-3 font-medium">
                  <Link href={`/workflows/${wf.id}/versions/${v.id}`} className="block text-[var(--accent)] underline">
                    {v.version}
                  </Link>
                </td>
                <td className="px-4 py-3">
                  <Link href={`/workflows/${wf.id}/versions/${v.id}`} className="block">
                ...
                <td className="px-4 py-3">
                  <Link href={`/workflows/${wf.id}/versions/${v.id}`} className="block text-[var(--muted)]">
                    {formatDate(v.updated_at)}
                  </Link>
                </td>
                <td className="px-4 py-3">
                  <Link href={`/workflows/${wf.id}/versions/${v.id}`} className="inline-block">
                    {v.status === "active" ? (
